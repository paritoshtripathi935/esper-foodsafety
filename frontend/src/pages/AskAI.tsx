...
interface AssistantMessage { id: string; role: 'assistant'; text: string; sources?: number }
interface ErrorMessage     { id: string; role: 'error';     text: string; retryPayload: AskPayload }
interface TypingMessage    { id: string; role: 'typing' }
type Message = UserMessage | AssistantMessage | ErrorMessage | TypingMessage

interface AskPayload { question: string; site_id: string; from: string; to: string }

interface ShellContext { events: FoodSafetyEvent[]; sites: Site[]; selectedSite: string | null }
...
function resolveRange(range: DateRange): { from: string; to: string } {
  const now = new Date()
  switch (range) {
    case 'last24h': return { from: subHours(now, 24).toISOString(), to: now.toISOString() }
    case 'today':   return { from: startOfDay(now).toISOString(),   to: endOfDay(now).toISOString() }
    case 'last7d':  return { from: startOfDay(subDays(now, 7)).toISOString(), to: endOfDay(now).toISOString() }
  }
}
...
interface StarterGridProps { onSelect: (q: string) => void; disabled: boolean }
...
interface BubbleProps { msg: Message; onRetry: (payload: AskPayload) => void }
...
export default function AskAI() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 502) {
          ...
          try { const j = await res.json(); errText = j.error ?? j.message ?? errText } catch { /* ignore */ }
        }
        setMessages((prev) => [
          ...prev.filter((m) => m.role !== 'typing'),
      ...
      setMessages((prev) => [
        ...prev.filter((m) => m.role !== 'typing'),
        { id: uid(), role: 'error', text: `Network error: ${(err as Error).message}`, retryPayload: payload },
      ])
    } finally {
      setLoading(false)
    }
  ...
  function submit(question: string) {
    ...
    const payload: AskPayload = { question: q, site_id: siteId || 'site-eastgate', from, to }
