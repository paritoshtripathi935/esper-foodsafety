...
export default function Overview() {
  ...
  const kpi = useMemo(() => {
    ...
    const onlineCount = stations.filter((s) => {
      ...
      const latest = Math.max(...stationEvents.map((e) => new Date(e.ts).getTime()))
      return now - latest < tenMin
    })
    const totalY = totalRegistryStations > 0 ? totalRegistryStations : stations.length
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {kpi.lastTs && (
            <span className="text-xs text-on-surface-variant">
              Last update{' '}
              {formatDistanceToNow(kpi.lastTs, { addSuffix: true })}
            </span>
          )}
        </div>
