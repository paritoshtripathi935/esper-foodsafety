...
const ZONE_LABEL = { cold: '❄ Cold', hot: '🔥 Hot', frozen: '🧊 Frozen', prep: '🥬 Prep' } as const
const ZONE_PILL = {
...
function ChartTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: { ts: string; value: number } }> }) {
  ...
  return (
    <div className="px-2.5 py-1.5 rounded-lg bg-surface-container border border-outline-variant text-xs shadow-lg">
      <span className="font-mono text-on-surface-variant">{format(parseISO(ts), 'HH:mm')}</span>
      <span className="mx-1.5 text-outline">·</span>
      <span className="font-mono font-semibold text-on-surface">{value.toFixed(1)}°F</span>
    </div>
  )
}
...
export default function StationDetail() {
  const { siteId, station: stationSlug } = useParams<{ siteId: string; station: string }>()
  ...
  const stationName =
    stationRecord?.name ??
    (stationSlug ? stationSlug.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase()) : '')
  ...
  // Last-24h temp data points for the chart
  const cutoff = useMemo(() => subHours(new Date(), 24).getTime(), [])
  ...
  const chartData = useMemo(
    () =>
      stationEvents
        ...
        .map((e) => ({ ts: e.ts, value: Number(e.value) })),
  ...
  const caDots = useMemo(
    () =>
      stationEvents.filter(
        (e) => e.type === 'corrective_action' && new Date(e.ts).getTime() >= cutoff
      ),
    [stationEvents, cutoff]
  )
  ...
  const yDomain = useMemo((): [number, number] => {
    ...
    if (stationRecord?.max_temp_f != null) values.push(stationRecord.max_temp_f)
    if (stationRecord?.min_temp_f != null) values.push(stationRecord.min_temp_f)
    if (values.length === 0) return [30, 50]
  ...
  // X-axis: tick every 2 hours, only label on the hour
  const xTicks = useMemo(() => {
    if (chartData.length === 0) return []
    const start = new Date(chartData[0].ts)
    const end = new Date(chartData[chartData.length - 1].ts)
    const ticks: number[] = []
    // Start at next even hour after first point
    const cursor = new Date(start)
    cursor.setMinutes(0, 0, 0)
    cursor.setHours(cursor.getHours() + (start.getMinutes() > 0 ? 2 : 0))
    ...
    <div className="flex flex-col gap-6 p-6 max-w-[1400px]">
          ...
          <ResponsiveContainer width="100%" height={320}>
            <LineChart data={chartData} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
              ...
              <XAxis
                ...
                tick={{ fontSize: 11, fill: 'var(--color-on-surface-variant, #888)' }}
                tickLine={false}
                axisLine={false}
              />
              ...
              {stationRecord?.max_temp_f != null && (
                <ReferenceLine
                  ...
                  strokeOpacity={0.6}
                  strokeDasharray="5 3"
                  label={{ value: `max ${stationRecord.max_temp_f}°F`, position: 'insideTopRight', fontSize: 10, fill: 'var(--color-error, #ef4444)' }}
                />
              )}
              {stationRecord?.min_temp_f != null && (
                <ReferenceLine
                  ...
                  strokeOpacity={0.6}
                  strokeDasharray="5 3"
                  label={{ value: `min ${stationRecord.min_temp_f}°F`, position: 'insideBottomRight', fontSize: 10, fill: 'var(--color-tertiary, #b45309)' }}
                />
              )}
              ...
              {alertDots.map((e) => {
                ...
                const nearest = chartData.reduce((best, d) =>
                  Math.abs(new Date(d.ts).getTime() - ts) < Math.abs(new Date(best.ts).getTime() - ts) ? d : best,
                  chartData[0]
                )
                return (
                  <ReferenceDot
      ...
      <div className="rounded-2xl border border-outline-variant bg-surface-container overflow-hidden">
        ...
        {recentEvents.length === 0 ? (
          <p className="px-5 py-4 text-sm text-on-surface-variant">No events yet for this station.</p>
        ) : (
              ...
              return (
                <Row
                  ...
                  className={clsx(
                    ...
                    isAlert && 'hover:bg-error-container/10 cursor-pointer transition-colors'
                  )}
                >
                  ...
                  <span className="text-on-surface-variant truncate">{eventDetail(e)}</span>
                  {isAlert && (
